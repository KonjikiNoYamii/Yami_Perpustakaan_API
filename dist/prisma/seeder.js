...
async function main() {
    ...
    const categories = await Promise.all(categoryNames.map((nama) => prisma.category.create({
        data: { nama },
    })));
    ...
    for (let i = 0; i < 80; i++) {
        ...
        const selectedBooks = faker.helpers.arrayElements(books, faker.number.int({ min: 1, max: 3 }));
